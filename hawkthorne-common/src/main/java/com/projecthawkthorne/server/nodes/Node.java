...
import com.projecthawkthorne.server.Direction;
...
public abstract class Node extends Collidable {
	...
	/** the server that this uses for socket communication */
	public static Server server = Server.getSingleton();
	...
	public void die() {
		...
		this.bb = null;
		String message = String.format("%s %s", this.id, "dead");
		server.send(message);
	}
	...
	public void setBound(MapObject obj) {
		...
		if (obj instanceof PolylineMapObject) {
			...
			for (int i = 0; i < size; i+=2) {
				...
				y[i] = Math.round(polylineObj.getPolyline().getVertices()[i+1]);
			}
			...
			if (this.properties.get("bbox_width") != null) {
				this.bbox_width = (Float)(this.properties
						.get("bbox_width"));
			}
			if (this.properties.get("bbox_height") != null) {
				this.bbox_height = (Float)(this.properties
						.get("bbox_height"));
			}

			if (this.bbox_width < 0 && this.properties.get("width") != null) {
				this.bbox_width = (Float)
	...
	/**
	 * packs the node for convenient socket transport into a string
	 * 
	...
	public String packFull() {
		...
		str = ("N" + ONE + "id" + ONE + this.getId() + NULL);
		buff.append(str);
		...
		str = ("S" + ONE + "state" + ONE
				+ this.getState().toString().toLowerCase() + NULL);
		buff.append(str);
		str = ("S" + ONE + "direction" + ONE
				+ this.direction.toString().toLowerCase() + NULL);
		buff.append(str);
		str = ("N" + ONE + "position" + ONE + this.position + NULL);
		buff.append(str);
		str = ("S" + ONE + "name" + ONE + this.name + NULL);
		buff.append(str);
		str = ("S" + ONE + "type" + ONE + this.type + NULL);
		buff.append(str);
		str = ("N" + ONE + "width" + ONE + this.width + NULL);
		buff.append(str);
		str = ("N" + ONE + "height" + ONE + this.height + NULL);
		buff.append(str);
	...
	public String packPartial() {
		StringBuilder buff = new StringBuilder();
		String str = ("N" + ONE + "id" + ONE + this.getId() + NULL);
		buff.append(str);
		str = ("N" + ONE + "x" + ONE + Math.round(this.x) + NULL);
		buff.append(str);
		str = ("N" + ONE + "y" + ONE + Math.round(this.y) + NULL);
	 ...
	 * 
	 * @param field
	 *            the desired field to send in a message
	 * @return
	 */
